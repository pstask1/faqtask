...
class Inix2Helper extends Helper
{
    ...
    public function createTemplate($tpl_name)
    {
        ...
        if ($this->module) {
            ...
            if (file_exists($this->module->getLocalPath() . 'views/templates/admin/' . $this->override_folder . $this->base_folder . $tpl_name)) {
                $override_tpl_path = $this->module->getLocalPath() . 'views/templates/admin/' . $this->override_folder . $this->base_folder . $tpl_name;
            } elseif (file_exists($this->module->getLocalPath() . 'views/templates/' . $this->override_folder . $this->base_folder . $tpl_name)) {
                $override_tpl_path = $this->module->getLocalPath() . 'views/templates/' . $this->override_folder . $this->base_folder . $tpl_name;
            } elseif (file_exists($this->module->getFrameLocalPath() . 'template/' . $this->base_folder . $tpl_name)) {
    ...
    $translations, $selected_cat = array(), $input_name = 'categoryBox', $use_radio = false, $use_search = false, $disabled_categories = array(), $use_in_popup = false
    )
    {
        ...
        return $helper->renderCategoryTree(
                        $root, $selected_cat, $input_name, $use_radio, $use_search, $disabled_categories, $use_in_popup
        );
    }

    /**
     *
     * @param array  $root                array with the name and ID of the tree root category, if null the Shop's root
    ...
    $root = null, $selected_cat = array(), $input_name = 'categoryBox', $use_radio = false, $use_search = false, $disabled_categories = array(), $use_in_popup = false, $use_shop_context = false
    )
    {
        $translations = array(
...
//			$html .= 'buildTreeView('.$use_shop_context.');';
     ...
     * @param string  $class
     * @param boolan  $addslashes   if set to true, the return value will pass through addslashes(). Otherwise,
     *                              stripslashes().
     * @param boolean $htmlentities if set to true(default), the return value will pass through htmlentities($string,
     *                              ENT_QUOTES, 'utf-8')
     *
