...
class Inix2HelperList extends Inix2Helper
{
    ...
    public function displayListContent()
    {
        if ($this->position_identifier) {
            $id_category = (int) Tools::getValue(
                'id_' . ($this->is_cms ? 'cms_' : '') . 'category',
                ($this->is_cms ? '1' : Category::getRootCategory()->id)
            );
        } else {
        ...
        if (isset($this->fields_list['position'])) {
            if ($this->position_identifier) {
                if (isset($this->position_group_identifier)) {
                    ...
                    $position_group_identifier = (int) Tools::getValue(
                        'id_' . ($this->is_cms ? 'cms_' : '') . 'category',
                        ($this->is_cms ? '1' : Category::getRootCategory()->id)
                    );
                }
        ...
        foreach ($this->_list as $index => $tr) {
            ...
            if ($this->shopLinkType) {
                $this->_list[$index]['short_shop_name'] = Tools::strlen($tr['shop_name']) > 15 ? Tools::substr(
                    $tr['shop_name'],
                    0,
                    15
                ) . '...' : $tr['shop_name'];
            ...
            foreach ($this->actions as $action) {
                ...
                if (!array_key_exists($action, $this->list_skip_actions) || !in_array(
                    $id,
                    $this->list_skip_actions[$action]
                )
                ) {
                    ...
                    if (method_exists($this->module, $method_name)) {
                        ...
                        $this->_list[$index][$action] = $this->context->controller->$method_name($this->token, $id,
                            $name, $tr);
            ...
            foreach ($this->fields_list as $key => $params) {
                ...
                if (isset($params['active'])) {
                    ...
                    $this->_list[$index][$key] = ImageManager::thumbnail(
                        $path_to_image,
                    ...
                    if (isset($params['callback'])) {
                        ...
                        $this->_list[$index][$key] = call_user_func_array(
                            array($callback_obj, $params['callback']),
                            array($echo, $tr)
                        );
                    } else {
    ...
    public function displayListHeader()
    {
        ...
        if (!isset($this->table_id) && $this->position_identifier && (int) Tools::getValue(
        ...
        if ($this->is_admin_controller) {
            $prefix = isset($this->controller_name) ? str_replace(
        ...
        foreach ($this->fields_list as $key => $params) {
            ...
            $value = Context::getContext()->cookie->{$prefix . $this->table . 'Filter_' . (array_key_exists(
                'filter_key',
                $params
