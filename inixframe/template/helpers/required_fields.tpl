...
<a class="btn btn-default" href="#" onclick="if ($('.requiredFieldsParameters:visible').length == 0) $('.requiredFieldsParameters').slideDown('slow'); else $('.requiredFieldsParameters').slideUp('slow'); return false;">
	<i class="icon-plus-sign"></i> {l s='Set required fields for this section' mod='inixframe'}
</a>
<div class="clearfix">&nbsp;</div>
<div style="display:none" class="panel requiredFieldsParameters">
	<h3><i class="icon-asterisk"></i> {l s='Required Fields' mod='inixframe'}</h3>
	<form name="updateFields" action="{$current}&amp;submitFields=1&amp;token={$token}" method="post">
		<div class="alert alert-info">
			{l s='Select the fields you would like to be required for this section.' mod='inixframe'}
		</div>
		<div class="row">
			<table class="table">
				<thead>
					<tr>
						...
						<th><span class="title_box">{l s='Field Name' mod='inixframe'}</span></th>
					</tr>
				</thead>
				<tbody>
		...
		<div class="panel-footer">
			<button name="submitFields" type="submit" class="btn btn-default pull-right">
				<i class="process-icon-save "></i> <span>{l s='Save' mod='inixframe'}</span>
			</button>					
		</div>
	</form>
</div>