...
class Inix2Module extends Module
{
    ...
    public function __construct($name = null, Context $context = null)
    {
        ...
        $this->admin_webpath = preg_replace(
                '/^' . preg_quote(DIRECTORY_SEPARATOR, '/') . '/', '', $this->admin_webpath
        );
    }
    ...
    public function processFilter()
    {
        ...
        foreach ($filters as $key => $value) {
            ...
            if ($value != null && !strncmp(
                            $key, $this->cookie_prefix . $this->object_table . 'Filter_', 7 + Tools::strlen($this->cookie_prefix . $this->object_table)
                    )
            ) {
    ...
    public function ajaxProcessRegister()
    {
        ...
        if (!Tools::isSubmit('update_service_password')) {
            ...
            $this->errors[] = sprintf(
                    $this->l('Password should be at least %d charactes long'), Validate::PASSWORD_LENGTH
            );
        }
        ...
        if (!count($this->errors)) {
            ...
            $response = $client->register(
                    Tools::getValue('update_service_email'), Tools::getValue('update_service_password')
            );
            ...
            if (isset($object->noZeroObject) && count(call_user_func(array(
                ...
                $this->errors[] = Tools::displayError('You need at least one object.') .
                        ...
                        Tools::displayError('You cannot delete all of the items.');
                            ...
                            $object->id, $this->list_skip_actions['delete']
                    )
                ...
                if ($this->deleted) {
                    ...
                    self::addLog(
                            sprintf($this->l('%s deletion', 'AdminTab', false, false), $this->className), 1, null, $this->className, (int) $this->object->id, true, (int) $this->context->employee->id
                    );
                }
            }
        } else {
    ...
    public function processAdd()
    {
        ...
        if (count($this->errors) <= 0) {
            ...
            } /* voluntary do affectation here */ elseif (($_POST[$this->object_identifier] = $this->object->id) && $this->postImage($this->object->id) && !count($this->errors) && $this->_redirect) {
                self::addLog(
                        sprintf($this->l('%s addition', 'AdminTab', false, false), $this->className), 1, null, $this->className, (int) $this->object->id, true, (int) $this->context->employee->id
                );
        ...
        if (empty($this->errors)) {
            ...
            if (isset($id) && !empty($id)) {
                ...
                if (Validate::isLoadedObject($object)) {
                    ...
                    self::addLog(
                            sprintf($this->l('%s edition', 'AdminTab', false, false), $this->className), 1, null, $this->className, (int) $object->id, true, (int) $this->context->employee->id
                    );
                } else {
                    $this->errors[] = Tools::displayError('An error occurred while updating an object.') .
    ...
    public function processStatus()
    {
        if (Validate::isLoadedObject($object = $this->loadObject())) {
            if ($object->toggleStatus()) {
                $matches = array();
                if (preg_match('/[\?|&]controller=([^&]*)/', (string) $_SERVER['HTTP_REFERER'], $matches) !== false && strtolower($matches[1]) != strtolower(preg_replace('/controller/i', '', get_class($this)))
                ) {
                    $this->redirect_after = preg_replace(
                            '/[\?|&]conf=([^&]*)/i', '', (string) $_SERVER['HTTP_REFERER']
                    );
                } else {
    ...
    public function processResetFilters()
    {
        ...
        foreach ($filters as $cookie_key => $filter) {
            if (strncmp(
                            $cookie_key, $this->cookie_prefix . $this->object_table . 'Filter_', 7 + Tools::strlen($this->cookie_prefix . $this->object_table)
                    ) == 0
            ) {
     ...
     * @return bool
     */
    public function installMailTpl($mails, $tpl_name)
    {
        //$mails = true;
        ...
        foreach ($langs as $l) {
            ...
            $mails &= @copy(
                            $this->getLocalPath() . '/mails/' . $tpl_name . '.html', $this->getLocalPath() . '/mails/' . $l['iso_code'] . '/' . $tpl_name . '.html'
            );
            if (file_exists($this->getLocalPath() . '/mails/' . $tpl_name . '.txt')) {
                $mails &= @copy(
                                $this->getLocalPath() . '/mails/' . $tpl_name . '.txt', $this->getLocalPath() . '/mails/' . $l['iso_code'] . '/' . $tpl_name . '.txt'
                );
            } else {
                $mails &= @copy(
                                $this->getLocalPath() . '/mails/' . $tpl_name . '.html', $this->getLocalPath() . '/mails/' . $l['iso_code'] . '/' . $tpl_name . '.txt'
                );
            }
        }
                    ...
                    if (isset($values['type']) && $values['type'] == 'textLang' or $values['type'] == 'textareaLang') {
                        ...
                        foreach ($languages as $language) {
                            ...
                            if (($value_default_lang or $value) && isset($values['validation'])) {
                                if (!call_user_func(['Validate', $validate_method], Tools::getValue(
                        ...
                        if (!call_user_func(['Validate', $validate_method], Tools::getValue($field))) {
    ...
    protected function processAddfeedback()
    {
        ...
        if (!count($this->errors)) {
            ...
            if (Inix2Mail::FrameSend(
                            'feedback', '[Feedback] ' . $this->author, $template_vars, Tools::getValue('email'), Tools::getValue('name'), null
                    )
            ) {
    ...
    protected function processAddBugreport()
    {
        ...
        if (!count($this->errors)) {
            ...
            if (Inix2Mail::FrameSend(
                            'bugreport', '[BUG REPORT] ' . $this->displayName . ' v' . $this->version, $template_vars, Tools::getValue('email'), Configuration::get('PS_SHOP_NAME'), $file_attachment, $this
                    )
            ) {
                $this->context->smarty->assign('conf', true);
            } else {
    ...
    public function initPageHeaderToolbar()
    {
            ...
            case 'edit':
                ...
                if (Validate::isLoadedObject($obj) && isset($obj->{$this->object_identifier_name}) && !empty($obj->{$this->object_identifier_name})) {
                    ...
                    $this->toolbar_title[] = sprintf(
                            $this->l('Edit: %s'), is_array($obj->{$this->object_identifier_name}) ? $obj->{$this->object_identifier_name}[$this->context->employee->id_lang] : $obj->{$this->object_identifier_name}
                    );
                }
                break;
        }

        if (is_array($this->page_header_toolbar_btn) && $this->page_header_toolbar_btn instanceof Traversable || count($this->toolbar_title)
        ) {
        ...
        $this->context->smarty->assign(
                'help_link', 'http://help.prestashop.com/' . $this->context->language->iso_code . '/doc/' . Tools::getValue('controller') . '?version=' . _PS_VERSION_ . '&country=' . $this->context->country->iso_code
        );
    }
    ...
    public function setHelperDisplay(Inix2Helper $helper)
    {
        ...
        $helper->title = is_array($this->toolbar_title) ? implode(
                        ' ' . Configuration::get('PS_NAVIGATION_PIPE') . ' ', $this->toolbar_title
                ) : $this->toolbar_title;
    ...
    {
        $this->handleMultiTables();


        // Manage list filtering
        if (Tools::isSubmit('submitFilter' . $this->object_table) || $this->context->cookie->{'submitFilter' . $this->object_table} !== false || Tools::getValue($this->object_table . 'Orderby') || Tools::getValue($this->object_table . 'Orderway')
        ) {
            $this->filter = true;
        }
        ...
        if (isset($_GET['deleteImage'])) {
        ...
        } /* Delete object */ elseif (isset($_GET['delete' . $this->object_table])) {
            $this->action = 'delete';
        } /* Change object statuts (active, inactive) */ elseif ((isset($_GET['status' . $this->object_table]) || isset($_GET['status'])) && Tools::getValue($this->object_identifier)) {
            $this->action = 'status';
        } /* Move an object */ elseif (isset($_GET['fposition'])) {
            $this->action = 'position';
        } elseif (Tools::isSubmit('submitAdd' . $this->object_table) || Tools::isSubmit('submitAdd' . $this->object_table . 'AndStay') || Tools::isSubmit('submitAdd' . $this->object_table . 'AndPreview') || Tools::isSubmit('submitAdd' . $this->object_table . 'AndBackToParent')
        ) {
            ...
            $this->action = 'export';
        } /* Cancel all filters for this tab */ elseif (isset($_POST['submitReset' . $this->object_table])) {
            $this->action = 'reset_filters';
        } /* Submit options list */ elseif (Tools::isSubmit('submitInixOptions' . $this->object_table) || Tools::isSubmit('submitInixOptions')) {
            $this->display = 'options';

            $this->action = 'update_options';
        } elseif (Tools::getValue('action') && method_exists(
                        $this, 'process' . ucfirst(Tools::toCamelCase(Tools::getValue('action')))
                )
        ) {
    ...
    public function getList(
            $id_lang, $order_by = null, $order_way = null, $start = 0, $limit = null, $id_lang_shop = false
    )
    {
        $prefix = $this->name;
        ...
        if ($this->multishop_context && Shop::isTableAssociated($this->object_table) && !empty($this->className)) {
            if (Shop::getContext() != Shop::CONTEXT_ALL || !$this->context->employee->isSuperAdmin()) {
                $test_join = !preg_match(
                                '#`?' . preg_quote(_DB_PREFIX_ . $this->object_table . '_shop') . '`? *sa#', $this->_join
                );
        ...
        if ($this->explicitSelect) {
            foreach ($this->fields_list as $key => $array_value) {
                ...
                if (isset($this->_select) && preg_match(
                                '/[\s]`?' . preg_quote($key, '/') . '`?\s*,/', $this->_select
                        )
                ) {
                    continue;
                }
                        ...
                        if (isset($input['type']) && $input['type'] == 'shop') {
                            if ($obj->id) {
                                $result = Shop::getShopById(
                                                (int) $obj->id, $this->object_identifier, $this->object_table
                                );
                ...
                if (isset($def['required']) && $def['required']) {
                    ...
                    if (empty($value)) {
                        ...
                        $this->errors[$field . '_' . $default_language->id] = sprintf(
                                Tools::displayError('The field %1$s is required at least in %2$s.'), $object->displayFieldName($field, $class_name), $default_language->name
                        );
                    }
                }
                ...
                foreach (Language::getLanguages(false) as $language) {
                    ...
                    if (!empty($value)) {
                        if (($error = $object->validateField(
                                $field, $value, $language['id_lang'], $skip, true
                                )) !== true
                        ) {
                ...
                foreach (Language::getLanguages(false) as $language) {
                    if (($value = Tools::getValue($field_lang . '_' . $language['id_lang'])) !== false && !empty($value)) {
                        if (Tools::strtolower($function) == 'iscleanhtml' && Configuration::get('PS_ALLOW_HTML_IFRAME')) {
                            ...
                            $res = call_user_func(['Validate', $function], $value, true);
                        } else {
                            $res = call_user_func(['Validate', $function], $value);
                        }
                        if (!$res) {
                            $this->errors[$field_lang . '_' . $language['id_lang']] = sprintf(
                                    Tools::displayError('The %1$s field (%2$s) is invalid.'), call_user_func(array($class_name, 'displayFieldName'), $field_lang, $class_name), $language['name']
                            );
                        }
                    }
                }
    ...
    protected function updateAssoShop($id_object)
    {
        ...
        Db::getInstance()->delete(
                $this->object_table . '_shop', '`' . $this->object_identifier . '` = ' . (int) $id_object . ($exclude_ids ? ' AND id_shop NOT IN (' . implode(
                                ', ', $exclude_ids
                        ) . ')' : '')
        );
            ...
            if ((!isset($field['empty']) || !$field['empty'] || (isset($field['empty']) && $field['empty'] && $value)) && $valid_method_exists) {
                if (isset($field['is_array']) and $field['is_array']) {
                    foreach ($value as $v) {
                        ...
                        if (!call_user_func(['Validate', $valid_method_name], $v)) {
                            $this->errors[] = Tools::displayError($field['title'] . ' : Has incorrect values');

                            return false;
                        }
                    }
                } elseif (!call_user_func(['Validate', $valid_method_name], $value)) {
    ...
    {
        if (isset($this->fieldImageSettings['prefix']) && $this->fieldImageSettings['prefix'] === 'md5class') {
            $id = md5($this->className) . '_' . $id;
        }
    ...
    protected function uploadImage($id, $name, $dir, $ext = false, $width = null, $height = null)
    {
        if (isset($_FILES[$name]['tmp_name']) && !empty($_FILES[$name]['tmp_name'])) {
            ...
            if (empty($this->errors) && !ImageManager::resize(
                            $tmp_name, _PS_IMG_DIR_ . $dir . md5($this->className) . '_' . $id . '.' . $this->imageType, (int) $width, (int) $height, ($ext ? $ext : $this->imageType)
                    )
            ) {
    ...
    protected function processBulkAffectZone()
    {
        ...
        if (is_array($this->boxes) && !empty($this->boxes)) {
            ...
            $result = $object->affectZoneToSelection(
                    Tools::getValue($this->object_table . 'Box'), Tools::getValue('zone_to_affect')
            );
    ...
    public function install()
    {
        ...
        foreach ($this->install_tabs as $t) {
            $tabs &= $this->installModuleTab(
                    $t['class'], $t['name'], $t['parent'], (isset($t['active']) ? $t['active'] : true), (isset($t['icon']) ? $t['icon'] : false)
            );
        }

        foreach ($this->install_mails as $m) {
            $mails = $this->installMailTpl(true, $m);
        }
     ...
     * @return bool
     */
    public function installModuleTab($tabClass, $tabName, $tabParent = 0, $active = true, $icon = false)
    {
        ...
        $result = $tab->save();
        if ($result) {
            $sql = 'UPDATE ' . _DB_PREFIX_ . 'tab SET icon="' . $icon . '" WHERE module="' . $this->name . '" AND class_name="' . $tabClass . '"';
    ...
    public function createTemplate($tpl_name)
    {
        ...
        if (file_exists($this->getLocalPath() . 'views' . DIRECTORY_SEPARATOR . 'templates' . DIRECTORY_SEPARATOR . 'admin' . DIRECTORY_SEPARATOR . $this->override_folder . $tpl_name)) {
            return $this->context->smarty->createTemplate(
                            $this->getLocalPath() . 'views' . DIRECTORY_SEPARATOR . 'templates' . DIRECTORY_SEPARATOR . 'admin' . DIRECTORY_SEPARATOR . $this->override_folder . $tpl_name, $this->context->smarty
            );
        }
        if (file_exists($this->getLocalPath() . 'views' . DIRECTORY_SEPARATOR . 'templates' . DIRECTORY_SEPARATOR . $this->override_folder . $tpl_name)) {
            return $this->context->smarty->createTemplate(
                            $this->getLocalPath() . 'views' . DIRECTORY_SEPARATOR . 'templates' . DIRECTORY_SEPARATOR . $this->override_folder . $tpl_name, $this->context->smarty
            );
        }

        return $this->context->smarty->createTemplate(
                        $this->getFrameLocalPath() . DIRECTORY_SEPARATOR . 'template' . DIRECTORY_SEPARATOR . $tpl_name, $this->context->smarty
        );
    }
    ...
    public static function addLog(
            $message, $severity = 1, $error_code = null, $object_type = null, $object_id = null, $allow_duplicate = false, $id_employee = null
    )
    {
     ...
     */
    private function setupPSOverride()
    {
        $setupResult = true;

        $overrideDir = $this->getLocalPath() . 'override';
        $moduleOverrideDir = $this->getPSOverrideDirectoryName(_PS_VERSION_, $overrideDir);

        // no override need
        if ($moduleOverrideDir == false) {
            return $setupResult;
        }

        // move and clean up
        $setupResult &= rename($overrideDir . '/' . $moduleOverrideDir, $this->getLocalPath() . $moduleOverrideDir);
        if ($setupResult) {
            Tools::deleteDirectory($overrideDir);
            $setupResult &= rename($this->getLocalPath() . $moduleOverrideDir, $overrideDir);
        }
    ...
    /**
     * Scan for directory that contains data for overriding the default PS functionality
     *
     * Note: The directory should be named like:
     * ps_vX.X.X.X
     * ps_vX.X.X
     * ps_vX.X
     * where X is a digit/number for example "ps_v1.7.0.6"
     *
     * @param string $psVersion   The version of the current PS instalation
     * @param string $overrideDir The aboslute path to the default PS module override directory
     *
     * @return string|bool The directory name that contains all the data for the module Override or FALSE if no override is needed.
     */
    private function getPSOverrideDirectoryName($psVersion, $overrideDir)
    {
        $dirName = false;
        ...
        $dirPrefix = 'ps_v';
        $dirBasePath = $overrideDir . '/' . $dirPrefix; // __DIR__ should be replaced with "<module-path>/override"
        $psVersionParts = explode('.', $psVersion);

        // X.X.X.X
        if (is_dir($dirBasePath . $psVersion)) {
            $dirName = $dirPrefix . $psVersion;
        }
        // X.X.X
        else if (is_dir($dirBasePath . $psVersionParts[0] . '.' . $psVersionParts[1] . '.' . $psVersionParts[2])) {
            $dirName = $dirPrefix . $psVersionParts[0] . '.' . $psVersionParts[1] . '.' . $psVersionParts[2];
        }
        // X.X
        else if (is_dir($dirBasePath . $psVersionParts[0] . '.' . $psVersionParts[1])) {
            $dirName = $dirPrefix . $psVersionParts[0] . '.' . $psVersionParts[1];
        }

        return $dirName;
    }

    private function handleMultiTables()
    {
        if (!isset($this->multiTables) || !$this->multiTables) {
            return;
        }
        ...
        foreach ($this->multiTables as $possKey => $table) {
            foreach ($_GET as $getKey => $value) {
                if (stristr($getKey, $table['table']) && !in_array($possKey, $found)) {
                    $results++;
                    $found[] = $possKey;
                }
                if (stristr($value, $table['table']) && !in_array($possKey, $found)) {
                    $results++;
                    $found[] = $possKey;
                }
            }
            foreach ($_GET as $getKey => $value) {
                if (stristr($getKey, $table['identifier']) && !in_array($possKey, $found)) {
                    $results++;
                    $found[] = $possKey;
                }
                if (stristr($value, $table['identifier']) && !in_array($possKey, $found)) {
                    $results++;
                    $found[] = $possKey;
                }
            }
            foreach ($_POST as $postKey => $value) {
                if (stristr($postKey, $table['table']) && !in_array($possKey, $found)) {
                    $results++;
                    $found[] = $possKey;
                }
                if (stristr($value, $table['table']) && !in_array($possKey, $found)) {
        ...
        if ($results === 1) {
            $this->setMultiTableChoice($found[0]);
            return;
        }
        die('ERROR :: Unhandled case');
    }

    protected function setMultiTableChoice($multiTableKey)
    {
        if (isset($this->multiTables[$multiTableKey])) {
            $this->className = $this->multiTables[$multiTableKey]['className'];
            $this->object_table = $this->multiTables[$multiTableKey]['table'];
            $this->object_identifier = $this->multiTables[$multiTableKey]['identifier'];
        }
    }

    protected function setMultiTables($multiTables)
    {
